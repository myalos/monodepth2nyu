...
def prepare_model_for_test(opt):
    ...
    encoder_path = os.path.join(f'log/{opt.model_name}/models/weights_{opt.num}', "encoder.pth")
    decoder_path = os.path.join(f'log/{opt.model_name}/models/weights_{opt.num}', "depth.pth")
...
def evaluate(opt):
    ...
    with torch.no_grad():
        for ind, (data, gt_depth, _, _, _, _) in enumerate(tqdm(dataloader)):
            ...
            disp = output['disp', 0]
            disp = F.interpolate(disp, (gt_depth.shape[2], gt_depth.shape[3]), mode='bilinear', align_corners=False)
